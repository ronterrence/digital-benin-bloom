...
4. Existing institution-specific Python and Node scripts normalize Oxford, British Museum, Met, and SMB source data; there was no unified workbook importer.
...
7. Sources and citations are stored as dataset-specific free-text fields and URLs.
8. Digital Benin links are sparse and dataset-specific rather than represented by one shared status model.
9. Vite writes the production bundle to `docs/` with the `/digital-benin-bloom/` base path for static hosting.
10. The safest migration is a separate, generated Atlas overlay. It leaves curated institutional datasets unchanged and publishes only sanitized fields.
...
Persistent researcher accounts, browser editing, approval workflows, and research-task management are deferred until the project adopts an authenticated backend.
