...
import researchQueueJson from "../data/normalized/atlas_research_queue.json";
import type { AtlasRecord, ResearchTask } from "@/data/atlasTypes";
import { label, recordsToCsv, summarizeInstitutions } from "@/lib/atlas";

const records = atlasJson as AtlasRecord[];
const researchQueue = researchQueueJson as ResearchTask[];
...
export default function AtlasPage() {
  ...
  return <main className="min-h-screen bg-background">
    <div className="mx-auto max-w-7xl px-4 pb-16 pt-16 md:px-6">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 text-sm text-muted-foreground"><p>Showing <span className="text-foreground">{filtered.length}</span> records across <span className="text-foreground">{summaries.length}</span> mapped institutions</p><div className="flex flex-wrap gap-2"><button type="button" className="rounded border border-border px-3 py-2" onClick={() => download("benin-atlas-filtered.json", JSON.stringify(filtered, null, 2), "application/json")}>Export JSON</button><button type="button" className="rounded border border-border px-3 py-2" onClick={() => download("benin-atlas-filtered.csv", recordsToCsv(filtered), "text/csv;charset=utf-8")}>Export CSV</button><button type="button" className="rounded border border-border px-3 py-2" onClick={() => download("benin-atlas-research-queue.json", JSON.stringify(researchQueue, null, 2), "application/json")}>Research queue ({researchQueue.length})</button></div></div>
