...
export const ATLAS_LABELS: Record<string, string> = {
  object: "Object", collection: "Collection lead", confirmed: "Confirmed", probable: "Probable", unclear: "Unclear",
  imported_unreviewed: "Imported — needs review", confirmed_1897_looted: "Confirmed 1897 link", probable_1897_looted: "Probable 1897 link",
  possible_1897_link: "Possible 1897 link", benin_object_1897_unproven: "1897 link unproven", not_1897_related: "Not 1897 related",
  matched_exact: "Digital Benin matched", matched_probable: "Probable Digital Benin match", not_checked: "Digital Benin not checked",
  not_found: "Not found on Digital Benin", not_in_scope: "Digital Benin not in scope", ownership_transferred_to_nigeria: "Ownership transferred",
  physically_returned_to_nigeria: "Physically returned", returned_but_on_loan_back: "Returned, on loan back", under_restitution_review: "Under review",
  still_owned_by_holding_museum: "Still held", status_unclear: "Ownership unclear", ownership_transferred: "Ownership transferred",
  physically_returned: "Physically returned", loaned_back: "Loaned back", under_review: "Under review", not_started: "Not started",
};
...
export function summarizeInstitutions(records: AtlasRecord[]): InstitutionSummary[] {
  ...
  return [...groups.values()]
    ...
    .map((items) => {
      const first = items[0];
      return {
        institutionId: first.institution_id, institutionName: first.institution_name, city: first.city, country: first.country,
        latitude: first.latitude!, longitude: first.longitude!, records: items,
...
export function recordsToCsv(records: AtlasRecord[]): string {
  const fields: (keyof AtlasRecord)[] = [
    "local_record_id", "record_level", "country", "city", "institution_name", "object_title", "object_type",
    "accession_number", "collection_identifier", "confidence_level", "expedition_1897_status", "digital_benin_match_status",
    "current_ownership_status", "restitution_status", "current_physical_location", "museum_catalogue_url", "digital_benin_url",
  ];
